...
inline void free_fader(fader_data_t *fader)
{
...
{
    fprintf(out, "fader {\n");
    print_channel_list(fader->channels, out);
    fprintf(out, "\t\t from:%d;\n", fader->from_value);
    fprintf(out, "\t\t to:%d;\n", fader->to_value);
    fprintf(out, "\t\t speed:%u;\n", fader->speed);
    fprintf(out, "\t}\n");
...
static void fade_in(const fader_data_t *fader)
{
    register int r = 1;
    while(r) {
        r = 0;
...
static void fade_out(const fader_data_t *fader)
{
    register int r = 1;
    while(r) {
        r = 0;
...
{
    if(fader->from_value < fader->to_value) {
        fade_in(fader);
    } else {
        fade_out(fader);
    }
